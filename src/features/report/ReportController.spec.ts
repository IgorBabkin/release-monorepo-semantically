...
describe('ReportController.generate', () => {
  ...
  it('given a configured matcher with packages "*" when generate runs then a docs(specs) commit patch-bumps every package', () => {
    const vsc = new Mock<VSCService>()
      .setup((m) => m.isWorkingTreeClean())
      .returns(true)
      .setup((m) => m.findManyCommitsSinceTag('pkg-a@1.0.0'))
    ...
    output.verify(
      (m) =>
        m.write(
          It.Is((body: string) => {
            const ctx = deserializeContext(body);
            return ctx.releasedVersions.get('pkg-a') === '1.0.1' && ctx.releasedVersions.get('pkg-b') === '2.0.1';
          }),
        ),
      Times.Once(),
    );
  ...
  it('given no configured bumps section when generate runs then behaviour matches the hard-coded default', () => {
    const vsc = new Mock<VSCService>()
      .setup((m) => m.isWorkingTreeClean())
      .returns(true)
      .setup((m) => m.findManyCommitsSinceTag('pkg-a@1.0.0'))
