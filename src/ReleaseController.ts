...
import {action, arg, controller, option} from './utils/controller';
...
export class ReleaseController {
    ...
    @action({default: true})
    async run(
        @arg(z.string()) path: string,
        @option('dry-run', z.boolean()) dryRun: boolean,
    ): Promise<void> {
        // Phase 1: Discovery
    ...
    private processPackage(pkg: NpmPackage, releasedVersions: Map<string, string>): PackageRelease | null {
        ...
        const releaseCommits = allCommits
            .filter((c) => c.matchesScope(pkg.name))
            .filter((c) => c.isReleaseTrigger());
    ...
    private printSummary(releases: PackageRelease[], dryRun: boolean): void {
        ...
        for (const release of releases) {
            const reason = release.dependencyUpdates.length > 0 && release.commits.length === 0
                ? 'dependency update'
